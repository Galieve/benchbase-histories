...
package com.oltpbenchmark.benchmarksHistory.tpccHistories.procedures;
...
import com.oltpbenchmark.benchmarksHistory.tpccHistories.TPCCWorkerHistory;
import com.oltpbenchmark.benchmarksHistory.tpccHistories.pojo.*;
import com.oltpbenchmark.historyModelHistory.events.*;
import com.oltpbenchmark.benchmarksHistory.tpccHistories.pojo.*;
...
public class NewOrderHistory extends TPCCProcedureHistory {

    private static final Logger LOG = LoggerFactory.getLogger(NewOrderHistory.class);
    ...
    """
        INSERT INTO %s
         (NO_O_ID, NO_D_ID, NO_W_ID, WRITEID)
         VALUES ( ?, ?, ?, ?)
         RETURNING *
    ...
    public void run(Connection conn, Random gen, int terminalWarehouseID, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, long startTime, TPCCWorkerHistory w, ArrayList<Event> events, int id, int soID) throws SQLException {
    ...
    private void insertNewOrder(Connection conn, int w_id, int d_id, int o_id, Integer id, Integer so, Integer po, ArrayList<Event> events) throws SQLException {
        ...
        try (PreparedStatement stmtInsertNewOrder = this.getPreparedStatement(conn, stmtInsertNewOrderSQL);) {
            ...
            stmtInsertNewOrder.setString(4, evID);
            stmtInsertNewOrder.execute();

            var rs = stmtInsertNewOrder.getResultSet();


            var no = new com.oltpbenchmark.benchmarksHistory.tpccHistories.pojo.NewOrderHistory();
