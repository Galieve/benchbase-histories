...
public class DeliveryHistory extends TPCCProcedureHistory {
    private static final Logger LOG = LoggerFactory.getLogger(DeliveryHistory.class);
    ...
    private void deleteOrder(Connection conn, int w_id, int d_id, int no_o_id, Integer id, Integer so, Integer po, ArrayList<Event> events) throws SQLException {
        try (PreparedStatement delivDeleteNewOrder = this.getPreparedStatement(conn, delivDeleteNewOrderSQL)) {
            ...
            events.add(new DeleteEvent(id, so, po, p.first, p.second, where, no.getTableNames()));
            if (result != 1) {
                ...
                String msg = String.format("NewOrderHistory delete failed. Not running with SERIALIZABLE isolation? [w_id=%d, d_id=%d, no_o_id=%d]", w_id, d_id, no_o_id);
