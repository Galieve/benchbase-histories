...
public class TPCCUtilHistory {

    /**
     * Creates a User object from the current row in the given ResultSet.
     ...
     */
    public static CustomerHistory newCustomerFromResults(ResultSet rs)
            throws SQLException {
    ...
    public static String getCurrentTime() {
        return TPCCConfigHistory.dateFormat.format(new java.util.Date());
    }
    ...
    public static int getItemID(Random r) {
        return nonUniformRandom(8191, OL_I_ID_C, 1, TPCCConfigHistory.configItemCount, r);
    }

    public static int getCustomerID(Random r) {
        return nonUniformRandom(1023, C_ID_C, 1, TPCCConfigHistory.configCustPerDist, r);
    }

    public static String getLastName(int num) {
        return TPCCConfigHistory.nameTokens[num / 100] + TPCCConfigHistory.nameTokens[(num / 10) % 10]
               + TPCCConfigHistory.nameTokens[num % 10];
    }

    public static String getNonUniformRandomLastNameForRun(Random r) {
        return getLastName(nonUniformRandom(255, C_LAST_RUN_C, 0, TPCCConfigHistory.configMaxDistinctCust - 1, r));
    }

    public static String getNonUniformRandomLastNameForLoad(Random r) {
        return getLastName(nonUniformRandom(255, C_LAST_LOAD_C, 0, TPCCConfigHistory.configMaxDistinctCust - 1, r));
    }
