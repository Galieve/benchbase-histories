...
package com.oltpbenchmark.benchmarksHistory.tpccPCHistories;
...
import com.oltpbenchmark.benchmarksHistory.tpccPCHistories.procedures.TPCCProcedureNDHistory;
...
public class TPCCPCWorkerHistory extends WorkerHistory<TPCCPCBenchmarkHistory> {

    private static final Logger LOG = LoggerFactory.getLogger(TPCCPCWorkerHistory.class);
    ...
    public TPCCPCWorkerHistory(TPCCPCBenchmarkHistory benchmarkModule, int id,
     ...
     */
    @Override
    protected TransactionStatus executeWorkHistory(Connection conn, TransactionType nextTransaction, ArrayList<Event> events, int id, int soID) throws UserAbortException, SQLException {
        try {
            var nxtTransactionHistory = (TransactionTypeHistory) nextTransaction;
            TPCCProcedureNDHistory proc =
                    (TPCCProcedureNDHistory) this.getProcedure(nextTransaction.getProcedureClass());
            proc.run(conn, gen, terminalWarehouseID, numWarehouses,
                    terminalDistrictLowerID, terminalDistrictUpperID,
                    nxtTransactionHistory.getStartTime(), this, events, id, soID);
