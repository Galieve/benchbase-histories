...
import com.oltpbenchmark.benchmarksHistory.tpccHistories.pojo.*;
import com.oltpbenchmark.benchmarksHistory.tpccPCHistories.pojo.NewOrderPCHistory;
 ...
 */
public class TPCCPCLoaderHistory extends Loader<TPCCPCBenchmarkHistory> {
    ...
    public TPCCPCLoaderHistory(TPCCPCBenchmarkHistory benchmark, int so) {
        super(benchmark);
            ...
            for (int d = 1; d <= districtsPerWarehouse; d++) {
                ...
                for (int c = 1; c <= customersPerDistrict; c++) {
                    ...
                    if (c >= FIRST_UNPROCESSED_O_ID) {
                        NewOrderPCHistory new_order = new NewOrderPCHistory();
                        new_order.no_w_id = w_id;
                        new_order.no_d_id = d;
                        new_order.no_o_id = c;
                        new_order.no_delivered = false;
                        ...
                        newOrderStatement.setBoolean(idx++, new_order.no_delivered);
