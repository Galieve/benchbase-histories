...
import com.oltpbenchmark.benchmarksHistory.tpccPCHistories.procedures.NewOrderPCHistory;
...
public class TPCCPCBenchmarkHistory extends BenchmarkModuleHistory {
    private static final Logger LOG = LoggerFactory.getLogger(TPCCPCBenchmarkHistory.class);

    public TPCCPCBenchmarkHistory(WorkloadConfiguration workConf) {
        super(workConf);
    }

    @Override
    protected Package getProcedurePackageImpl() {
        return (NewOrderPCHistory.class.getPackage());
    }
    ...
    protected List<Worker<? extends BenchmarkModule>> makeWorkersImpl() {
        ...
        try {
            List<TPCCPCWorkerHistory> terminals = createTerminals();
    ...
    @Override
    protected Loader<TPCCPCBenchmarkHistory> makeLoaderImplHistory(int so) {
        return new TPCCPCLoaderHistory(this, so);
    }

    protected List<TPCCPCWorkerHistory> createTerminals() throws SQLException {

        TPCCPCWorkerHistory[] terminals = new TPCCPCWorkerHistory[workConf.getTerminals()];
        ...
        for (int w = 0; w < numWarehouses; w++) {
            ...
            for (int terminalId = 0; terminalId < numWarehouseTerminals; terminalId++) {
                ...
                TPCCPCWorkerHistory terminal = new TPCCPCWorkerHistory(this, workerId++, w_id, lowerDistrictId, upperDistrictId, numWarehouses);
