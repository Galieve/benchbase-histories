...
import com.oltpbenchmark.benchmarksHistory.seatsHistories.pojo.ConfigProfile;
...
import com.oltpbenchmark.benchmarksHistory.seatsHistories.pojo.ConfigHistograms;
import com.oltpbenchmark.benchmarksHistory.seatsHistories.procedures.Config;
...
public class SEATSProfileHistory {
    private static final Logger LOG = LoggerFactory.getLogger(SEATSProfileHistory.class);
    ...
    protected final SEATSBenchmarkHistory benchmark;
    ...
    public SEATSProfileHistory(SEATSBenchmarkHistory benchmark, RandomGenerator rng) {
     ...
     */
    protected final void saveProfile(Connection conn, ArrayList<Event> events, int so, int po) throws SQLException {
        ...
        profileSql = profileSql + " RETURNING *";

        try (PreparedStatement stmt = conn.prepareStatement(profileSql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
            ...
            stmt.setString(param_idx, evID);
            stmt.execute();

            var rs = stmt.getResultSet();
            var p = new ConfigProfile();
            var writeSet = p.getInsertEventInfo(rs);
            events.add(new InsertEvent(0, so, po, writeSet, p.getTableNames()));
            ++po;
        ...
        histogramSql = histogramSql + " RETURNING *";
        try (PreparedStatement stmt = conn.prepareStatement(histogramSql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
            for (Entry<String, Histogram<String>> e : this.airport_histograms.entrySet()) {
                ...
                var rs = stmt.getResultSet();
                var h = new ConfigHistograms();
                var writeSet = h.getInsertEventInfo(rs);
                events.add(new InsertEvent(0, so, po, writeSet, h.getTableNames()));
                ++po;
    ...
    private SEATSProfileHistory copy(SEATSProfileHistory other) {
     ...
     */
    private static SEATSProfileHistory cachedProfile;

    protected final ArrayList<Event> loadProfile(SEATSWorkerHistory worker, int id, int so) throws SQLException {
        synchronized (SEATSProfileHistory.class) {
            var events = new ArrayList<Event>();
            // Check whether we have a cached Profile we can copy from
            if (cachedProfile != null) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Using cached SEATSProfileHistory");
                }
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Loading SEATSProfileHistory for the first time");
            }

            // Otherwise we have to go fetch everything again
            LoadConfigHistory proc = worker.getProcedure(LoadConfigHistory.class);

            Config results;
            try (Connection conn = benchmark.makeConnection()) {
                conn.setAutoCommit(false);
                results = proc.run(conn, events, id, so);
                conn.commit();
            }
            // CONFIG_PROFILE
            ...
            cachedProfile = new SEATSProfileHistory(this.benchmark, this.rng).copy(this);
            return events;
        }
    }
