...
import com.oltpbenchmark.historyModelHistory.LoaderThreadHistory;
...
public class SEATSLoaderHistory extends Loader<SEATSBenchmarkHistory> {
    // -----------------------------------------------------------------
    ...
    public SEATSLoaderHistory(SEATSBenchmarkHistory benchmark, int so) {
        super(benchmark);
        this.so = so;
    ...
    protected FixedDataIterable getFixedIterable(Table catalog_tbl) throws Exception {
        String path = SEATSBenchmarkHistory.getTableDataFilePath(this.profile.airline_data_dir, catalog_tbl);
        ...
        public FixedDataIterable(Table catalog_tbl, String filePath) throws Exception {
            ...
            for (Column catalog_col : catalog_tbl.getColumns()) {
                ...
                if (catalog_col.getName().toUpperCase().contains("_SATTR")) {
                    ...
                    this.rnd_string_min.put(col_idx, SEATSLoaderHistory.this.rng.nextInt(catalog_col.getSize() - 1));
            ...
            return (new TableIterator() {
                ...
                public Object[] next() {
                    ...
                    for (int col_idx : FixedDataIterable.this.rnd_string) {
                        ...
                        tuple[col_idx] = SEATSLoaderHistory.this.rng.astring(min_length, max_length);
                    }
                    // Random Integer (*_IATTR##)
                    for (int col_idx : FixedDataIterable.this.rnd_integer) {
                        tuple[col_idx] = SEATSLoaderHistory.this.rng.nextLong();
                    }
                ...
                public Object[] next() {
                    ...
                    for (int i = 0; i < ScalingDataIterable.this.data.length; i++) {
                        ...
                        // Strings
                            ...
                            int min_len = SEATSLoaderHistory.this.rng.nextInt(max_len - 1);
                            ScalingDataIterable.this.data[i] = SEATSLoaderHistory.this.rng.astring(min_len, max_len);

                        // Ints/Longs
                        } else {
                            ScalingDataIterable.this.data[i] = SEATSLoaderHistory.this.rng.number(0, 1 << 30);
                        }
                    }
    ...
    protected class CustomerIterable extends ScalingDataIterable {
        ...
        private final Flat randBalance;
        ...
        public CustomerIterable(Table catalog_tbl, long total) {
            ...
            this.randBalance = new Flat(SEATSLoaderHistory.this.rng, 1000, 10000);
        }
            ...
            switch (columnIdx) {
                ...
                case (0): {
                    ...
                    while (airport_id == null) {
                        ...
                        airport_id = SEATSLoaderHistory.this.profile.getAirportId(this.airport_code);
                    }
                    int next_customer_id = SEATSLoaderHistory.this.profile.incrementAirportCustomerCount(airport_id);
                    ...
                    if (LOG.isTraceEnabled()) {
                        LOG.trace("{} => {} [{}]", value, this.airport_code, SEATSLoaderHistory.this.profile.getCustomerIdCount(airport_id));
                    }
                    break;
                }
                // CUSTOMER ID STR
    ...
    protected class FrequentFlyerIterable extends ScalingDataIterable {
        ...
        public FrequentFlyerIterable(Table catalog_tbl, long num_customers) {
            ...
            this.customer_id_iterator = new CustomerIdIterable(SEATSLoaderHistory.this.profile.airport_max_customer_id).iterator();
            ...
            SEATSLoaderHistory.this.flights_per_airline.putAll();
            this.airline_rand = new FlatHistogram<>(SEATSLoaderHistory.this.rng, SEATSLoaderHistory.this.flights_per_airline);
            if (LOG.isTraceEnabled()) {
                this.airline_rand.enableHistory();
            }
            if (LOG.isDebugEnabled()) {
                LOG.debug("Flights Per Airline:\n{}", SEATSLoaderHistory.this.flights_per_airline);
            }
            ...
            switch (columnIdx) {
                ...
                case (0): {
                    while (this.customer_idx < this.ff_per_customer.length && this.ff_per_customer[this.customer_idx] <= 0) {
                        ...
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(String.format("CUSTOMER IDX: %d / %d", this.customer_idx, SEATSLoaderHistory.this.profile.getCustomerIdCount()));
                        }
    ...
    protected class AirportDistanceIterable extends ScalingDataIterable {
        ...
        public AirportDistanceIterable(Table catalog_tbl, int max_distance) {
            ...
            this.num_airports = SEATSLoaderHistory.this.airport_locations.size();
            this.record_airports = SEATSLoaderHistory.this.profile.getAirportCodes();
        }
        ...
        protected boolean hasNext() {
            for (; this.outer_ctr < (this.num_airports - 1); this.outer_ctr++) {
                this.outer_airport = SEATSLoaderHistory.this.airport_locations.get(this.outer_ctr);
                this.outer_location = SEATSLoaderHistory.this.airport_locations.getValue(this.outer_ctr);
                if (!SEATSLoaderHistory.this.profile.hasFlights(this.outer_airport)) {
                    continue;
                }
                ...
                for (; inner_ctr < this.num_airports; inner_ctr++) {
                    ...
                    this.inner_airport = SEATSLoaderHistory.this.airport_locations.get(inner_ctr);
                    this.inner_location = SEATSLoaderHistory.this.airport_locations.getValue(inner_ctr);
                    if (!SEATSLoaderHistory.this.profile.hasFlights(this.inner_airport)) {
                        continue;
                    }
                    ...
                    if (this.record_airports.contains(this.outer_airport) && this.record_airports.contains(this.inner_airport)) {
                        SEATSLoaderHistory.this.setDistance(this.outer_airport, this.inner_airport, this.distance);
                    }
    ...
    protected class FlightIterable extends ScalingDataIterable {
        ...
        public FlightIterable(Table catalog_tbl, int days_past, int days_future) {
            ...
            Gaussian gauss_rng = new Gaussian(SEATSLoaderHistory.this.rng, 0, all_airlines.size());
            ...
            for (String airport_code : histogram.values()) {
                ...
                this.flights_per_airport.put(airport_code, new FlatHistogram<>(SEATSLoaderHistory.this.rng, histogram));
            }
            ...
            SEATSLoaderHistory.this.profile.setFlightStartDate(this.start_date);
            ...
            // Update profile
            SEATSLoaderHistory.this.profile.setFlightPastDays(days_past);
            SEATSLoaderHistory.this.profile.setFlightFutureDays(days_future);
        }
        ...
        private void populate(Timestamp date) {
            ...
            this.arrive_time = SEATSLoaderHistory.this.calculateArrivalTime(this.depart_airport, this.arrive_airport, this.depart_time);

            // Airline
            this.airline_code = this.airlines.nextValue();
            this.airline_id = SEATSLoaderHistory.this.profile.getAirlineId(this.airline_code);
            ...
            switch (columnIdx) {
                ...
                case (0): {
                    ...
                    while (true) {
                        ...
                        this.flight_id = new FlightId(this.airline_id, SEATSLoaderHistory.this.profile.getAirportId(this.depart_airport), SEATSLoaderHistory.this.profile.getAirportId(this.arrive_airport), this.start_date, this.depart_time);
                    ...
                    SEATSLoaderHistory.this.addFlightId(this.flight_id);
                ...
                case (1): {
                    ...
                    SEATSLoaderHistory.this.flights_per_airline.put(this.airline_code);
                    break;
                }
                // DEPART AIRPORT
                case (2): {
                ...
                case (9): {
                    ...
                    for (int seatnum = 0; seatnum < SEATSConstantsHistory.FLIGHTS_NUM_SEATS; seatnum++) {
                        ...
                        SEATSLoaderHistory.this.decrementFlightSeat(this.flight_id);
                    }
                    value = (long) SEATSLoaderHistory.this.getFlightRemainingSeats(this.flight_id);
                    if (LOG.isTraceEnabled()) {
    ...
    protected class ReservationIterable extends ScalingDataIterable {
        ...
        public ReservationIterable(Table catalog_tbl, long total) {
            ...
            for (long airport_id : SEATSLoaderHistory.this.profile.getAirportIds()) {
        ...
        private void generateData() throws Exception {
            ...
            for (FlightId flight_id : SEATSLoaderHistory.this.getFlightIds()) {
                long depart_airport_id = flight_id.getDepartAirportId();
                String depart_airport_code = SEATSLoaderHistory.this.profile.getAirportCode(depart_airport_id);
                long arrive_airport_id = flight_id.getArriveAirportId();
                String arrive_airport_code = SEATSLoaderHistory.this.profile.getAirportCode(arrive_airport_id);
                Timestamp depart_time = flight_id.getDepartDateAsTimestamp(SEATSLoaderHistory.this.profile.getFlightStartDate());
                Timestamp arrive_time = SEATSLoaderHistory.this.calculateArrivalTime(depart_airport_code, arrive_airport_code, depart_time);
                ...
                if (LOG.isTraceEnabled()) {
                    ...
                    m.put("Departure", String.format("%s / %s", SEATSLoaderHistory.this.profile.getAirportCode(depart_airport_id), depart_time));
                    m.put("Arrival", String.format("%s / %s", SEATSLoaderHistory.this.profile.getAirportCode(arrive_airport_id), arrive_time));
                ...
                for (int seatnum = 0; seatnum < booked_seats; seatnum++) {
                    ...
                    Integer airport_customer_cnt = SEATSLoaderHistory.this.profile.getCustomerIdCount(depart_airport_id);
                    ...
                    while (tries > 0) {
                        ...
                        // airport
                            ...
                            customer_id = SEATSLoaderHistory.this.profile.getRandomCustomerId(depart_airport_id);
                        }
                        // New Outbound Reservation
                        // We'll take anybody!
                        else {
                            customer_id = SEATSLoaderHistory.this.profile.getRandomCustomerId();
                        }
        ...
        private void getReturningCustomers(Collection<ReturnFlight> returning_customers, FlightId flight_id) {
            Timestamp flight_date = flight_id.getDepartDateAsTimestamp(SEATSLoaderHistory.this.profile.getFlightStartDate());
        ...
        protected Object specialValue(long id, int columnIdx) {
            ...
            switch (columnIdx) {
                ...
                case (2): {
                    ...
                    if (SEATSLoaderHistory.this.profile.getReservationUpcomingOffset() == null && flight_id.isUpcoming(SEATSLoaderHistory.this.profile.getFlightStartDate(), SEATSLoaderHistory.this.profile.getFlightPastDays())) {
                        SEATSLoaderHistory.this.profile.setReservationUpcomingOffset(id);
                    }
                    break;
                }
                // SEAT
        ...
        return (new Iterable<FlightId>() {
            ...
            public Iterator<FlightId> iterator() {
                return (new Iterator<FlightId>() {
                    private int idx = 0;
                    private final int cnt = SEATSLoaderHistory.this.seats_remaining.size();
                    ...
                    public FlightId next() {
                        return (SEATSLoaderHistory.this.seats_remaining.get(this.idx++));
                    }
