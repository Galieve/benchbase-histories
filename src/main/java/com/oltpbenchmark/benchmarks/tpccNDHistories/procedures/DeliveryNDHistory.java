...
import com.oltpbenchmark.benchmarks.tpccHistories.pojo.CustomerHistory;
...
public class DeliveryNDHistory extends TPCCProcedureNDHistory {
    private static final Logger LOG = LoggerFactory.getLogger(DeliveryNDHistory.class);
    ...
    public void run(Connection conn, Random gen, int w_id, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, long startTime, TPCCWorkerNDHistory w, ArrayList<Event> events, int id, int soID) throws SQLException {
    ...
    private void deleteOrder(Connection conn, int w_id, int d_id, int no_o_id, Integer id, Integer so, Integer po, ArrayList<Event> events) throws SQLException {
        ...
        try (PreparedStatement delivDeleteNewOrder = this.getPreparedStatement(conn, delivDeleteNewOrderSQL)) {
            ...
            Function<Value, Value> set = (val)->{
                val.setValue("NO_DELIVERED", String.valueOf(true));
                val.setValue("WRITEID", writeID);
                return val;
            };

            var p = no.getUpdateEventInfo(set, rs);
            Function<Value, Boolean> where = (val) ->
                val != null &&
            ...
            if (result != 1) {
                ...
                String msg = String.format("NewOrderNDHistory delete failed. Not running with SERIALIZABLE isolation? [w_id=%d, d_id=%d, no_o_id=%d]", w_id, d_id, no_o_id);
