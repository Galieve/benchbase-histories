...
import com.oltpbenchmark.benchmarks.tpccNDHistories.pojo.NewOrderNDHistory;
 ...
 */
public class TPCCLoaderNDHistory extends Loader<TPCCBenchmarkNDHistory> {
    ...
    public TPCCLoaderNDHistory(TPCCBenchmarkNDHistory benchmark, int so) {
        super(benchmark);
            ...
            for (int d = 1; d <= districtsPerWarehouse; d++) {
                ...
                for (int c = 1; c <= customersPerDistrict; c++) {
                    ...
                    if (c >= FIRST_UNPROCESSED_O_ID) {
                        NewOrderNDHistory new_order = new NewOrderNDHistory();
