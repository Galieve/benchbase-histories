...
function create_image() {
    ...
    cp -a ../../../artifact-scripts/smoke-test.sh tmp/smoke-test.sh
    ...
    local image_name='benchbase-artifact'
    if [ "$profiles" == "$default_profile" ]; then
        # This is a singleton image, mark it as such.
        image_name="benchbase-artifact-${default_profile}"
    fi
